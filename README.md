...
2. 点 **`ebook_reader-1.0.0.apk`** → 点页面里的 **View raw** → 下载
3. 点它安装；系统提示「未知来源」时选**仍要安装**
   （它没有正式证书，是自签名包 —— 详见下方「关于签名」）
4. 再下载同目录的 **`世说新语.epub`**（186 KB，真实古籍，推荐用它试读）
5. 打开 App → **导入 EPUB** → 在文件选择器里选它 → 点书名开始阅读

> `release/README.md` 里有每个文件的说明与《世说新语》的来源/权利声明。
